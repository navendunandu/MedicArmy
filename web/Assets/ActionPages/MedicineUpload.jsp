<%-- 
    Document   : MedicineUpload
    Created on : 19 Sep, 2024, 3:22:33 PM
...
//checking if request cotains multipart data
        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("image"))//write the name of image here it is img_product
                {
                    ...
                    String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\MedicineDocs\\";//path for storing image
                    Random r = new Random();//to generate random number
                    int r_num = r.nextInt(1111) + 999;//values between this are generated

                    image = "medicineImage" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
    ...
    String InsQry = "insert into tbl_medicine(medicine_name,medicine_description,medicine_image,subcategory_id,medicine_rate,shop_id) "
            + "values('" + value[0] + "','" + value[1] + "','" + image + "','" + value[3] + "','" + value[4] + "','" + session.getAttribute("sid") + "')";
...
<script type="text/javascript">
    ...
    setTimeout(function() {
        window.location.href = '../../MedicalShop/Medicine.jsp'
