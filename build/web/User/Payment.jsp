...
<html lang="en" >
    ...
    if(request.getParameter("btnpay")!=null)
    {
        ...
        String up ="update tbl_appointment set appointment_status='"+request.getParameter("st")+"' where appointment_id='"+request.getParameter("aid")+"'";
        if(con.executeCommand(up))
        {
            %>
      <script type="text/javascript">
            alert("Payment Complated");
            setTimeout(function(){window.location.href='ViewAppointmentList.jsp'},100);
        </script>
     <%
        }
        }
        else{
            String app = "SELECT max(appointment_id) as id from tbl_appointment where user_id="+session.getAttribute("uid");
            ResultSet ap = con.selectCommand(app);
            ap.next();
            String up ="update tbl_appointment set appointment_status='1' where appointment_id='"+ap.getInt("id")+"'";
            if(con.executeCommand(up))
        {
            %>
      <script type="text/javascript">
