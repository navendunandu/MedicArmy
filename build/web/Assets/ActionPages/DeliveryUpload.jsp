<%-- 
    Document   : DeliveryUpload
    Created on : 19 Sep, 2024, 3:10:32 PM
...
//checking if request cotains multipart data
        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("proof"))//write the name of image here it is img_product
                {
                    ...
                    String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\DeliveryDocs\\";//path for storing image
                    Random r = new Random();//to generate random number
                    int r_num = r.nextInt(1111) + 999;//values between this are generated

                    proof = "deliveryProof" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                ...
                if (field_name.equals("photo"))//write the name of image here it is img_product
                {
                    ...
                    photo = "deliveryPhoto" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                    //creating a file object
    ...
    String InsQry =  "insert into tbl_deliveryboy(deliveryboy_name,deliveryboy_address,deliveryboy_email,deliveryboy_password,deliveryboy_contact,deliveryboy_photo,deliveryboy_proof,place_id,shop_id) "
            + "values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + value[4] + "','" + photo + "','" + proof + "','" + value[6] + "','"+ session.getAttribute("sid") +"')";
...
<script type="text/javascript">
    ...
    setTimeout(function() {
        window.location.href = '../../Guest/DeliveryBoyRegistration.jsp'
